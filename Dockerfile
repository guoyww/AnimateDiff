...
RUN echo "conda activate animatediff" > ~/.bashrc
ENV PATH /opt/conda/envs/env/bin:$PATH
RUN conda run -n animatediff pip install xformers accelerate imageio[pyav] imageio[ffmpeg] --no-cache-dir
ENTRYPOINT ["/opt/conda/envs/animatediff/bin/python", "app.py"]
