...
from scripts.utils.n3r_utils import patchify_latents, unpatchify_latents, generate_frame_patched_v3, load_images
...
def main(args):
    ...
    vae = safe_load_vae_stable(args.pretrained_model_path, device="cpu", fp16=False, offload=True)
    vae.to(device)
    ...
    output_dir = Path(f"./outputs/hybrid26_{timestamp}")
    ...
    with open(csv_file, "w", newline="") as f_csv:
        ...
        writer.writerow(["frame", "warnings"])
        ...
        for img_path in input_paths:
            ...
            for f_idx in range(num_fraps_per_image):
                ...
                # ---- batching des patchs ----
                pos_emb, neg_emb = embeddings[0]
                frame_tensor = generate_frame_patched_v3(
                    ...
                    pos_emb=pos_emb,
                    neg_emb=neg_emb,
                    tile_size=64,           # plus petit pour réduire mémoire
                    overlap=16,
                    steps=steps,
    ...
    print("✅ Génération patch-based optimisée terminée.")
